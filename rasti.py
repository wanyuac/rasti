...
def parse_arguments():
    ...
    parser.add_argument('--pause', '-p', dest = 'pause', type = float, default = 0.2, required = False, help = "Number of seconds to hold between consecutive BLAST searches (Default: 0.2; range: 0-60)")
...
def main ():
    ...
    out_dirs = {'root' : args.outdir, \
                'blast' : os.path.join(args.outdir, '1_blast'), \
                'parsed' : os.path.join(args.outdir, '2_parsed'),\
                'extended' : os.path.join(args.outdir, '3_extended')}
    for d in out_dirs.values():
        SanityCheck.output_dir(d)
    if SanityCheck.output_file(f = args.query, message = True):
        blast = BLAST(query_fasta = args.query, \
                      min_identity = SanityCheck.parameter_range(v = args.min_identity, v_min = 70.0, v_max = 100.0, v_reset = 80.0, n = 'min_identity'),\
                      min_qcov = SanityCheck.parameter_range(v = args.min_qcov, v_min = 0, v_max = 100.0, v_reset = 80.0, n = 'min_qcov'), max_evalue = args.max_evalue,\
                      max_hits = SanityCheck.parameter_range(v = args.max_match_num, v_min = 1, v_max = 200, v_reset = 5, n = 'max_match_num'))
        blast.check_executives()
    else:
        sys.exit(1)
    genomes = SanityCheck.fasta_files(fs = args.genomes, suf = '.' + args.assembly_suffix)  # Return: {genome name : Path of the assembly's FASTA file}
    ...
    if n > 0:
        ...
        sys.exit(1)
    delay_sec = args.pause  # Number of seconds to hold between consecutive BLAST searches
